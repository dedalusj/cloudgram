...
describe('refresh', () => {
  ...
  beforeEach(async () => {
    ...
    const index = await import('../src/ts/index');
    refresh = index.draw;
    editor = index.editor;
  });
...
describe('link', () => {
  ...
  beforeEach(async () => {
    ...
    // we await import the index after mocking the document
    // body so that ace can find the editor div
    const index = await import('../src/ts/index');
...
describe('save graph', () => {
  ...
  it('saves the graph as png', async () => {
    document.body.innerHTML = `
      ...
      <div id="editor"></div>
    `;
    (await import('../src/ts/index')).saveGraph();

    expect(cy.png).toHaveBeenCalledWith({output: 'blob', scale: 1, full: true});
    expect(fileSaver.saveAs).toHaveBeenCalledWith(blob, expect.stringMatching(/\.png$/));
  });

  it('saves the graph as jpg', async () => {
    document.body.innerHTML = `
      <select id="format">
        <option>png</option>
    ...
    expect(fileSaver.saveAs).toHaveBeenCalledWith(blob, expect.stringMatching(/\.jpeg$/));
  });

  it('saves the graph as svg', async () => {
    document.body.innerHTML = `
      <select id="format">
        <option>png</option>
        <option>jpeg</option>
    ...
    expect(fileSaver.saveAs).toHaveBeenCalledWith(svgBlob, expect.stringMatching(/\.svg$/));
  });
});
